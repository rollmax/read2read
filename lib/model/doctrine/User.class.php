...
class User extends GuardUser
{
  ...
  {
    if($this->getIsBlocked()) return false;
...
//    if( $this->getTariff()=='expert' || $this->getTariff()=='super' ) return true;
  ...
  public function setNewTariff($tariffName)
  {
    if($this->getTariff() == $tariffName && $this->getTariffChange() == 'none')
      return true;
    else if($this->getTariff() == $tariffName && $this->getTariffChange() != 'none')
  ...
  {
    return ($this->getUserpic() !== null) ? sfConfig::get('app_userpic_url_dir') . $this->getUserpic() : sfConfig::get('app_userpic_url_dir') . sfConfig::get('app_default_userpic_url') ;
  }
  ...
  public function addFunds($amount = 0)
  {
    ...
    if(!$addTransaction = $transaction->addFunds($this->getUSerTypePrefix(), $amount, $this->getId(), $this->getBalans()))
      return false;
  ...
  public function tryUnBlockUser()
  {
    ...
    {
      if($this->balans >= 2 * $settings->getPrice1k()) $res = true;
    }
    elseif($this->getUtype()=='puser')
    {
      $min_summ = $settings->getPriceStandart();
      if($this->getTariff() == 'expert') $min_summ = $settings->getPriceExpert();
      elseif($this->getTariff() == 'super') $min_summ = $settings->getPriceSuper();
      
      if($this->balans >= $min_summ) $res = true;
    }
  ...
  {
    if($this->getBalanceUser()->count()) return $this->getBalanceUser()->getFirst()->getSellPurchaseCnt();
  }
  ...
  {
    if($this->getBalanceUser()->count())return $this->getBalanceUser()->getFirst()->getAmount();
  }
  ...
  public function generateImg()
  {
    ...
    imagerectangle($img, 1, 1, $width - 1, 99, imagecolorallocate($img, rand(10, 255), rand(10, 255), rand(10, 255)));
  
    for($x = 2; $x < $width - 1; $x++) {
      imageline($img, $x, 2, $x, $height - 2, imagecolorallocate($img, rand(20, 255), rand(0, 255), rand($x, 255)));
    }
  
    // Save file
    $this->setImgFileName(md5($this->getId().'Ab222+!+R').'.png');
    $file_full_path = sfConfig::get('sf_upload_dir').sfConfig::get('app_secret_img_path_dir').'/'.$this->getImgFileName();
  ...
  {
    if($this->getImgFileMd5() != md5_file($fileName)) return false;
    if($this->getImgFileSize() != filesize($fileName)) return false;
    return true;
  }
}