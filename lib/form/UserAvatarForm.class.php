...
class UserAvatarForm extends UserForm
{
  ...
  {
    if ($values['avatar'] instanceof sfValidatedFile)
    { // file was uploaded
      $path = $this->getObject()->checkDir($values['avatar']->getPath(), $this->getObject()->getId());
      $values['avatar'] = $this->processPicture($values['avatar'], $path, 240, 400);
    }
    unset($values['avatar']);
  ...
  protected function processPicture(sfValidatedFile $file, $path, $width, $height, $crop = false)
  {
    ...
    $imageLocation = str_replace(sfConfig::get('sf_upload_dir').sfConfig::get('app_avatars_path_dir'), '', $image);
