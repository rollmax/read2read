...
<?php if(!$sf_user->isAuthenticated()) : ?>
  <?php slot('menu_part_1', '<td class="act_noreg"><a href="'.url_for('articles_category', $article->getCategory()).'"><p>Parts - Разделы</p></a></td>') ?>
<?php endif; ?>
...
<?php else : ?>
<a href="<?php echo url_for('articles_category', $article->getCategory()); ?>"><p><img src="/images/close_v.png" alt=""></p></a>
<?php endif; ?>
</div>
  ...
  <tr>
    ...
    <td class=blg-print>
      ...
      <?php if($sf_user->getGuardUser()->isPurchaseArticle($article->getId())): ?>
        <br/>
        <a  class=print2 href="<?php echo url_for('article_print', array('id' => $article->getId())) ?>" target="_blank"><p>Версия для печати</p></a>
      <?php endif; ?>
      <?php endif; ?>
    </td>
    <td>
  ...
  <tr>
    ...
    <td class=center>
      ...
      <?php if($sf_user->getGuardUser()->isPurchaseArticle($article->getId())): ?>
        ...
        <?php if (false === $sf_user->getGuardUser()->hasRateContent($article->getId()) || false === $sf_user->getGuardUser()->hasRateTranslate($article->getId())): ?>
        ...
        <?php if (false === $sf_user->getGuardUser()->hasRateContent($article->getId())): ?>
          ...
          <div class="rateit" data-rateit-backingfld="#art_rating" data-rateit-resetable="false" id="art_rating_div_user"></div>
        <?php else: ?>
          <p class=lt_ev>Оценка статьи</p>
          <select name="art_rating" class="rating" id="art_rating_rated">
            <option value="0">0</option>
            <?php for($i=1; $i<=5; $i++):?>
              <option value="<?php echo $i ?>" <?php echo ( $i == floor( $article->getContRate() ) ) ? 'selected' : '' ;?>>
                <?php echo $i ?>
              </option>
            <?php endfor;?>
          </select>
          
          <div class="rateit" data-rateit-backingfld="#art_rating_rated" data-rateit-resetable="false" data-rateit-readonly="true"></div>
        <?php endif; ?>
        ...
        <?php if (false === $sf_user->getGuardUser()->hasRateTranslate($article->getId())): ?>
          ...
          <div class="rateit" data-rateit-backingfld="#trans_rating" data-rateit-resetable="false" id="trans_rating_div_user"></div>
        <?php else:?>  
          <p class=lt_ev>Оценка перевода</p>
          <select name="trans_rating" class="rating" id="trans_rating_rated">
            <option value="0">0</option>
            <?php for($i=1; $i<=5; $i++):?>
              <option value="<?php echo $i ?>" <?php echo ( $i == floor( $article->getTransRate() ) ) ? 'selected' : '' ;?>>
                <?php echo $i ?>
              </option>
            <?php endfor;?>
          </select>
          
          <div class="rateit" data-rateit-backingfld="#trans_rating_rated" data-rateit-resetable="false" data-rateit-readonly="true"></div>
        <?php endif; ?>
...
<!-- admin rating   -->
      <?php if($sf_user->isAuthenticated() && $sf_user->hasPermission('admin')):?>
        ...
        <div class="rateit" data-rateit-backingfld="#art_rating2" data-rateit-resetable="false" id="art_rating_div"></div>
        ...
        <div class="rateit" data-rateit-backingfld="#trans_rating2" data-rateit-resetable="false" id="trans_rating_div"></div>
