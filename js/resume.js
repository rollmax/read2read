var resume = {
  ...
  {
    var html = '';
    html += (resume.data.firstName != null) ? '<p>Имя</p><p class=p>'+resume.data.firstName+'</p>' : '';
    html += (resume.data.lastName != null) ? '<p>Фамилия</p><p class=p>'+resume.data.lastName+'</p>' : '';
    html += (resume.data.place != null) ? '<p>Место жительства</p><p class=p>'+resume.data.place+'</p>' : '';
    html += (resume.data.email != null) ? '<p>e-mail</p><p class=p>'+resume.data.email+'</p>' : '';
    html += (resume.data.skype != null) ? '<p>skype</p><p class=p>'+resume.data.skype+'</p>' : '';
    html += (resume.data.phone != null) ? '<p>тел</p><p class=p>'+resume.data.phone+'</p>' : '';
    html += (resume.data.site != null) ?'<p>сайт</p><p><a target="_blank" href="'+resume.data.site+'">'+resume.data.site+'</a></p>' : '';
    html += (resume.data.resumeText != null) ? '<div id="text"><p class="txt">'+resume.data.resumeText+'</p></div>' : '';
    html += (resume.data.tariff != null) ?'<p>Тарифный план</p><p class="p">'+resume.data.tariff+'</p>' : '';
    return html;
  },
